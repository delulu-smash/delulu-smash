...
Cell output that's a `PIL.Image.Image` (e.g. a Python cell returning one)
renders as a real image via Kitty's own Terminal Graphics Protocol
(`kitty_image.py` — no Sixel/halfblock/other-protocol fallback, see that
module's docstring for why). This needs a terminal that actually speaks
that protocol (Ghostty does) — **and needs to be run in that terminal
directly**, not through an editor's integrated terminal panel (e.g.
VSCode's), which typically doesn't implement it.
...
- Execution is `exec`/`eval` in-process (see `kernel.py`), not a real
  Jupyter kernel — no cross-process isolation and no async/await support at
  the top level. A `PIL.Image.Image` result renders as a real image (see
  [Run](#run) above), but a plotting library's own figure object
  (matplotlib, etc.) isn't auto-converted to one yet — that'd need its own
  `display.py` case (e.g. `fig.savefig()` into a buffer, then
  `PIL.Image.open()` it) alongside `image_view`'s existing polars/pandas
  dispatch.
