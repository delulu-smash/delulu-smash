...
AI mode's `AI_ICON` is the one deliberate exception: Devicons has no
"AI"/robot concept at all (it's a tech-stack icon set -- languages,
frameworks, databases -- not agents), so per the rule above it pulls
